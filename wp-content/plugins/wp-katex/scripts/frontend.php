...
function katex_handler( $atts, $content = null ) {
	...
	if ( $displayAttr === "true" || $displayAttr === "1" || strpos( $content, '\\displaystyle' ) === 0 ) {
		return '<span class="inline-math katex-display" data-display="true">' . $enc . '</span>';
	} else {
		return '\(' . $enc . '</span>';
	}
}
...
function katex_rubber() {
	...
	if ( $katex_using ) {
		// @formatter:off
		?><script>!function(){"use strict";for(var e=document.querySelectorAll(".inline-math"),t=0;t<e.length;t++){var r=e[t],a=document.createElement("span");try{katex.render(r.textContent,a,{displayMode:"true"===r.getAttribute("data-display"),throwOnError:!1})}catch(n){a.style.color="red",a.textContent=n.message}r.parentNode.replaceChild(a,r)}}();</script><?php
		// @formatter:on
	}
}
